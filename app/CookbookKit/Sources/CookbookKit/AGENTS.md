...
- **The full-catalog sync sends an explicit high `limit`.** `RecipeQuery.all` carries `limit=100000` (NOT an empty query) so `syncCatalog`'s `recipes(.all)` pull returns EVERY recipe. A no-limit `GET /recipes` falls back to a server-side default that silently capped the sync at 1000 — "Recipes cached" froze at 1k once the library passed it while ~1500 sat server-side. An explicit limit is passed straight through to SQL, so this works against the deployed backend with no redeploy. Do NOT revert `.all` to `RecipeQuery()`.
