...
- `Screens/` — `HomeView`, `BrowseView`, `RecipeDetailView`, `AssistantView`, `PantryView`, `PlannerView`, `SavedView`, `SettingsView`, `PlaceholderScreen`, `HomePreviewData` (demo seed). (There is no Import screen — adding/importing is the Assistant; import status is `ActivityView`.)
...
- **`JobsList` is the single ingestion-jobs renderer (DRY).** It owns `JobRow` + the `IngestStage` timeline; the Activity sheet (`ActivityView`, raised from the Assistant) uses it. Per-row delete is a **swipe** action with **no confirm** (it only drops a history row via `IngestionStore.deleteJob`); **"Clear finished"** *is* confirmed (`IngestionStore.clearFinished`, terminal-only). `DONE` rows do **not** auto-expand their timeline; `ERROR` rows stay visible. The `IngestStage` timeline starts at `.uploading` (the optimistic pre-upload row, byte `%`), then `loading` shows live per-page movement ("Reading pages 3/12"); a dedup re-upload renders "Already in your library" (terminal `stage=skipped`). `progressDetail` shows `%` while uploading, `done/total` otherwise.
- **The Assistant is the unified ask + ADD surface (the IA decision).** `AssistantView` does two jobs in one chat: **ask** the cookbook (`recipeStore.ask` → `/ask`, prose bubbles + recipe-reference chips) and **add a recipe**, which must stay OBVIOUS / first-class. A persistent ＋/attach control in the composer offers the four add paths — *describe a recipe*, *find a recipe online*, *add from URL*, *attach PDF* — and the placeholder invites it ("Ask, paste a link, or describe a recipe to add").
  - **`ask` now sends the transcript as history** (built from `messages`, error bubbles skipped) so "make it spicier" / "save that one" / "delete #2" resolve against earlier turns. The agent itself can now add/delete/edit via tools (`save_recipe`/`delete_recipe`/`remove_ingredient`); `RecipeStore.ask` reconciles the catalog afterward so those changes appear. This is a CONVERSATIONAL add path (draft-in-prose → user confirms → `save_recipe`) that runs PARALLEL to the visual ＋ "describe a recipe" → `DraftRecipeCard` + Save path — both end at the same server save core; neither persists without explicit confirmation.
...
- **"Attach PDF" uses the EXISTING ingest path — never a draft.** It routes to `IngestionStore.ingestPDF(...)` (the async ingest job) and points the cook at the Activity sheet. macOS also accepts a PDF dropped on the window (`RootView.handleWindowDrop` → same `ingestPDF`). Do NOT build a second PDF path or render PDFs as compose drafts (the async ingest path already persists + polls). **The Assistant posts an inline outcome message** — "Importing X… (track in Activity)" on success, or "Couldn't import X: <reason>" on failure — rather than a banner that fired even when the upload never started; `startPDFIngest` branches on `ingestPDF`'s `Bool` result.
- **There is no Import destination.** The standalone Import screen was removed (Jun 2026): adding is the Assistant (describe / find / URL / PDF), and import *status* is the `ActivityView` sheet raised from the Assistant. Don't reintroduce an Import tab/sidebar row — extend the Assistant + Activity instead.
