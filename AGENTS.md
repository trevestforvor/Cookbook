...
Top-level folders without their own DOX (owned by root): `data/` (the SQLite DB, raw/interim/seed corpora, FDC nutrition CSVs), `scripts/` (one-off ingest entry points + `dox_check.py`, the DOX drift guard, and `hooks/pre-commit`), `tests/` (pytest over the backend; incl. `test_assistant_edit_tools.py` — agent `save_recipe`/`delete_recipe`/`remove_ingredient` + conversation-history threading), `research/`, `docs/` (`ARCHITECTURE.md`, `docs/MCP_SERVER.md`, `docs/ui-proposal.md`, `docs/design-recipes-compose.md` — the compose builder spec), `config.yaml`, `pyproject.toml`, `.dockerignore` (backend image build context), `.github/workflows/dox.yml` (CI enforcement) + `build-backend.yml` (builds/pushes the backend image to GHCR **and smoke-tests that it actually starts** — catches runtime import crashes a `docker build` misses; see `deploy/AGENTS.md`).
