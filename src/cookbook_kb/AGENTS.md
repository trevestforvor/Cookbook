...
- **One registry, defined once.** Every capability is a LAYER-1 function registered exactly once in `tools.py`. Never duplicate a verb; add it to `functions/` and register it. The agent advertises only `RECIPE_TOOL_SCHEMAS` (the **13** recipe tools — read/search/import + the three add/edit verbs `save_recipe`, `delete_recipe`, `remove_ingredient`); the MCP server exposes the full merged surface (recipe + harness). Keep that split.
- **The agent is conversational + can mutate the catalog.** `agent.run` threads prior turns (`history=`, capped at 20) so it can resolve "that one"/"number 2" and multi-step edits; `/ask` and the clients resend the transcript (server stays stateless). Its write verbs reuse the existing cores — `save_recipe`→`ingest.pipeline.ingest_one_recipe` (the shared compose/save path), `delete_recipe`/`remove_ingredient`→`store.recipes_admin` — and EACH bumps the catalog version so clients re-sync. Destructive/add verbs require explicit user confirmation (system-prompt contract); the agent never claims a save/delete a tool didn't return.
