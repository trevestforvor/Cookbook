...
- **Scanned PDF pages extract from the page IMAGE, not OCR text.** When `config.VLM_EXTRACTION` (default on; `config.yaml` `llm.vlm_extraction`) and a candidate spans scanned pages, `ingest.pipeline.ingest_one_pdf` calls `extract.extractor.extract_recipe_from_image` (multimodal `eagle-nothink`, ≤2 page PNGs from `loader.Page.image_png`) instead of `extract_recipe(text)`. Measured far more accurate than Tesseract on 2-column layouts (which mis-OCR'd 360 kcal → 3560 on ~⅓ of sampled pages) at ~the same latency. **OCR text still drives boundary segmentation**; the text extractor is the per-candidate FALLBACK on any image-extract miss. Requires vision enabled on the served model (image limit ≥1) + a backend image redeploy to take effect.
- **`loader.load` reports per-PAGE progress.** `progress("loading", done, total)` fires per page so the slow OCR phase shows live movement (`Reading pages 3/12`) instead of one frozen "loading"; the worker preserves a dedup re-drop as a terminal `stage=skipped` (client → "Already in your library").
