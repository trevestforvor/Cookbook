...
- **Long work is async, never inline.** PDF ingest and corpus backfills go through `jobs.py`/`worker.py`, not a blocking request handler. The pool size is `config.JOB_WORKERS` (default 8, env `COOKBOOK_JOB_WORKERS`, clamped 1..32) — matched to the served model's vLLM `max-num-seqs` so parallel ingests get proxy slots instead of piling up, and a couple of stuck jobs can't block the whole queue (a hardcoded 2 once froze every import). A hung LLM call no longer strands a worker for ~30 min: the `OpenAI()` client is bounded (`config.LLM_REQUEST_TIMEOUT`, default 180s, + 1 retry), so it errors in minutes and frees the slot. **And on startup `create_app` runs `ingest_jobs.fail_orphaned`** — the in-memory store is empty on a fresh process and the worker never resumes a durable row, so any row left `running`/`queued` by a prior process is flipped to a terminal `error` ("interrupted — server restarted") instead of sitting "running" in the app forever (`GET /ingest` merges these durable rows; that's how a restart left zombie jobs frozen). **Startup also runs `store.load.apply_dedup` (gated by `config.DEDUP_ON_STARTUP`, default on)** — the batch ingest pipeline NEVER marks duplicates, so re-uploads that slipped past the file-SHA skip (a frozen job never recorded its hash) pile up as visible dupes. `apply_dedup` re-derives `canonical_id` over the whole library with a conservative key (near-identical title AND ingredient-overlap ≥0.85 AND same variant); it only MARKS rows (the list query filters `canonical_id IS NULL`), so it's non-destructive + reversible — true identical re-uploads vanish from the catalog on the next boot, while near-dupes whose extractions drifted stay visible for the user to review. It owns `canonical_id` wholesale (resets then re-derives), so it's the single source of truth.
- **`GET /recipes` with no params returns the ENTIRE catalog — never a capped page.** The app's `recipes(.all)` sync sends no `limit` and expects every recipe; the handler uses `_ALL_LIMIT = -1` (SQLite `LIMIT -1` = no bound). Do NOT "tidy" this into a fixed number — a prior `_ALL_LIMIT = 1000` silently truncated the sync once the library passed 1k (rows in the DB, never sent → the app's "Recipes cached" froze at 1000). An explicit `?limit=N` is still honored for callers that want a page.
- **Ingest jobs stream live stage progress.** `worker.run_job` mirrors `progress(stage, done, total)` onto the in-memory job AND the durable row each milestone, so polling `GET /ingest/{id}` shows movement: `loading` is now per-PAGE (via `loader.load`), then `extracting`/`normalizing`/`embedding`. A dedup re-upload (the pipeline returns `[]` after `progress("skipped", …)`) keeps a terminal `stage=skipped` (not `done`) so the client can say "Already in your library" instead of "Done / 0 recipes".
- **`POST /ingest` accepts an optional client `job_id` form field.** When present, `JobStore.create` adopts it instead of generating one, so the app can seed an optimistic "uploading" row under the SAME id it will poll — the job appears the instant the cook picks a file, before the (large) multipart upload finishes. Backward-compatible: omit it and the server generates one as before (the client then reconciles its optimistic row to the returned id).
