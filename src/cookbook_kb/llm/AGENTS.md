...
- **Two model aliases, same endpoint + keys, different roles** (Gemma 4 26B-A4B; 128K window):
  - **`eagle-nothink` (`CHAT_MODEL`)** — guided-JSON extraction/generation (`extract_json`) and plain chat. Guided decoding needs the budget for JSON, not reasoning; the thinking variant spends it on reasoning and returns empty content with `finish_reason=length`. **Keep extraction/generation on no-think.**
  - **`eagle` (`REASONING_MODEL`)** — the agentic ReAct loops (`agent.run`, `web_researcher.run`). Tool-calling benefits from reasoning; the gemma4 reasoning parser routes thinking into a separate `reasoning_content` channel, so `tool_calls`/`content` come through clean. **Must raise `max_tokens` (`REASONING_MAX_TOKENS`, default 8192)** or reasoning truncates the tool-call (the empty-content/`length` failure). Trade-off: noticeably slower + higher latency variance than no-think — mind the client's 60s timeout.
