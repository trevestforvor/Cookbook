...
- **find by free web search** (`mode_hint: "find"`, no URL) → **WIRED (no-persist).** `subagents.web_researcher.find_recipe_draft_online` does ONE Brave search (`_web_search`) and walks the top results through `parse_recipe_from_url` (no load), returning the first that passes the `is_recipe` gate as a draft. Needs `BRAVE_API_KEY`; if it's unset or nothing parses, compose falls back to **generate** with a `warning`. Intent is explicit: `auto` with no URL still generates (so "make me a chili" generates); only the app's "Find a recipe online" affordance sends `mode_hint: "find"`. Unlike the agent's `research_recipes_online`/`web_researcher.run` (which import = persist), this never writes — the user reviews/edits and only `/compose/save` commits.
